...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    const prompt = `You are an expert educator. Generate ${count} educational flashcards about: "${topic}". Respond ONLY with a valid JSON array.`;
    ...
    aiCards.forEach((card: any, i: number) => {
      const id = uuidv4();
      const newCard = {
        id, deckId,
